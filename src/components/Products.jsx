    ...
    <li key={item.id} className='w-[326px] p-[18px] relative mb-[30px]'>
      ...
      <button onClick={() => dispatch({ type: "ADD_WISHLIST", payload: item })}>
        {state.wishlist?.some(i => i.id === item.id) ? (
          <FaHeart className='fa absolute top-0 right-[55px] text-red-500' />
        ) : (
          <FaRegHeart className='fa absolute top-0 right-[55px]' />
        )}
      </button>
      ...
      <div className='flex items-center gap-[116px]'>
        ...
        <button 
          ...
          className='bt w-[55px] cursor-pointer py-[5px] px-[20px] bg-[#454545] rounded-2xl'>
    ...
    <div>
      ...
      {show && selectedItem && (
        <Modal onClose={() => setShow(false)}>
          ...
          <button className='btr w-[250px] py-[10px] bg-[#454545] text-[30px] text-white rounded-lg ml-[140px] mb-[30px]' onClick={() => navigate(`/product/${selectedItem.id}`)}>Узнать больше</button>
          <br />
        </Modal>
      )}
    </div>
